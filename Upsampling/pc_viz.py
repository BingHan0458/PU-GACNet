...
def show_pointclouds(points, colors, text=[],
                     title="Default", png_path="", interactive=True,
    ...
    renders = [vtk.vtkRenderer() for _ in range(num_pointclouds)]
    ...
    for i, pc in enumerate(points):
        ...
        renders[i].AddActor(pointclouds[i].vtkActor)
        # renders[i].AddActor(vtk.vtkAxesActor())
        renders[i].SetBackground(1.0, 1.0, 1.0)
        if orientation == 'horizontal':
            print(viewports[i][0])
            renders[i].SetViewport(viewports[i][0], 0.0, viewports[i][1], 1.0)
        elif orientation == 'vertical':
            renders[i].SetViewport(0.0, viewports[i][0], 1.0, viewports[i][1])
        else:
            raise Exception('Not a valid orientation!')
        renders[i].ResetCamera()

    # Add circle to first render
    # renders[0].AddActor(getActorCircle())
    # renders[0].AddActor(getActorCircle(50, 49, color=(0, 1, 0)))
...
def get_points_colors_from_obj(filename):
    points = []
    with open(filename) as f:
        for line in f:
            parts = line.strip().split()
            points.append(np.array([float(parts[0]), float(parts[1]), float(parts[2])]))
    points = np.array(points)
    return points


def visualize_pointcloud(comparison_folder_list, filename, color, text=[], png_path="",
    ...
    for folder in comparison_folder_list:
        pts = get_points_colors_from_obj(os.path.join(folder, filename))
        all_points.append(pts)
        all_colors.append(np.multiply(np.ones(pts.shape), color))
    print(np.asarray(all_points).shape)
    show_pointclouds(all_points, all_colors,
                     text=text,
                     png_path=png_path, interactive=interactive,
