...
from GACNet import GAC_feature_extraction
...
class PUGAC(object):
    ...
    def __call__(self, inputs):
        with tf.variable_scope(self.name, reuse=self.reuse):
            # feature extraction using Graph Attention Convolution
            # graph_prd, idx = build_init_graph(inputs,self.opts.graph_inf)
            # features = build_network(graph_prd, self.opts.net_inf, inputs, self.opts.k_in_gac,tf.constant(True), None)

            # feature extraction
            features = GAC_feature_extraction(inputs, growth_rate=12, dense_n=3, knn=self.opts.k,
                       scope='feature_extraction2', is_training=self.is_training, bn_decay=None)
