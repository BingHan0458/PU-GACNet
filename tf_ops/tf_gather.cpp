...
class GatherPointGpuOp: public OpKernel{
  public:
    ...
    void Compute(OpKernelContext * context)override{
      ...
      OP_REQUIRES(context,inp_tensor.dims()==3,errors::InvalidArgument("GatherPoint2 expects (batch_size,num_points,channel) inp shape"));
      int b=inp_tensor.shape().dim_size(0);
      int n=inp_tensor.shape().dim_size(1);
      int channel=inp_tensor.shape().dim_size(2);
      const Tensor& idx_tensor=context->input(1);
      OP_REQUIRES(context,idx_tensor.dims()==2 && idx_tensor.shape().dim_size(0)==b,errors::InvalidArgument("GatherPoint2 expects (batch_size,num_result) idx shape"));
...
REGISTER_KERNEL_BUILDER(Name("GatherPoint2").Device(DEVICE_GPU),GatherPointGpuOp);
...
REGISTER_KERNEL_BUILDER(Name("GatherPointGrad2").Device(DEVICE_GPU),GatherPointGradGpuOp);
...
class GroupPointGpuOp: public OpKernel{
    public:
        ...
        void Compute(OpKernelContext * context) override {
            ...
            OP_REQUIRES(context, points_tensor.dims()==3, errors::InvalidArgument("GroupPoint2 expects (batch_size, num_points, channel) points shape"));
            int b = points_tensor.shape().dim_size(0);
            int n = points_tensor.shape().dim_size(1);
            int c = points_tensor.shape().dim_size(2);

            const Tensor& idx_tensor=context->input(1);
            OP_REQUIRES(context,idx_tensor.dims()==3 && idx_tensor.shape().dim_size(0)==b, errors::InvalidArgument("GroupPoint2 expects (batch_size, npoints, nsample) idx shape"));
            int m = idx_tensor.shape().dim_size(1);
...
REGISTER_KERNEL_BUILDER(Name("GroupPoint2").Device(DEVICE_GPU),GroupPointGpuOp);
...
class GroupPointGradGpuOp: public OpKernel{
    public:
        ...
        void Compute(OpKernelContext * context) override {
            const Tensor& points_tensor=context->input(0);
            OP_REQUIRES(context, points_tensor.dims()==3, errors::InvalidArgument("GroupPointGrad2 expects (batch_size, num_points, channel) points shape"));
            int b = points_tensor.shape().dim_size(0);
            int n = points_tensor.shape().dim_size(1);
            int c = points_tensor.shape().dim_size(2);

            const Tensor& idx_tensor=context->input(1);
            OP_REQUIRES(context,idx_tensor.dims()==3 && idx_tensor.shape().dim_size(0)==b, errors::InvalidArgument("GroupPointGrad2 expects (batch_size, npoints, nsample) idx shape"));
            int m = idx_tensor.shape().dim_size(1);
            int nsample = idx_tensor.shape().dim_size(2);
...
REGISTER_KERNEL_BUILDER(Name("GroupPointGrad2").Device(DEVICE_GPU),GroupPointGradGpuOp);
