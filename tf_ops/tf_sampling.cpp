...
REGISTER_OP("FarthestPointSample2")
  .Attr("stride: int")
  .Input("inp: float32")
  .Input("initid: int32")
  .Output("out: int32")
...
class FarthestPointSampleGpuOp: public OpKernel{
  public:
    explicit FarthestPointSampleGpuOp(OpKernelConstruction* context):OpKernel(context) {
                    ...
                    OP_REQUIRES(context, stride_ > 0, errors::InvalidArgument("FarthestPointSample2 expects positive stride"));
                }
    void Compute(OpKernelContext * context)override{
      
      const Tensor& inp_tensor=context->input(0);
      OP_REQUIRES(context,inp_tensor.dims()==3 && inp_tensor.shape().dim_size(2)==3,errors::InvalidArgument("FarthestPointSample2 expects (batch_size,num_points,3) inp shape"));
      int b=inp_tensor.shape().dim_size(0);
      int n=inp_tensor.shape().dim_size(1);
      auto inp_flat=inp_tensor.flat<float>();
      const float * inp=&(inp_flat(0));
...
REGISTER_KERNEL_BUILDER(Name("FarthestPointSample2").Device(DEVICE_GPU),FarthestPointSampleGpuOp);
