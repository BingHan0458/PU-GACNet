...
from ops import normalize_point_cloud
...
def analyze_uniform(idx_file, radius_file, map_points_file):
    ...
    for j in range(rad_number):
        ...
        for i in range(sample_number):
            ...
            expect_d = math.sqrt(2 * disk_area / 1.732)
...
with tf.Session() as sess:
    ...
    for D in [PRED_DIR]:
        ...
        with open(os.path.join(FLAGS.save_path, "evaluation.csv"), "w") as f:
            ...
            if global_uniform:
                global_uniforms = np.array(global_uniform)
                uniform_mean = np.mean(global_uniforms, axis=0)
            ...
            row["EMD (1e-3)"] = avg_emd_value*1000.
            if global_p2f:
                global_p2fs = np.concatenate(global_p2f, axis=0)
                mean_p2f = np.nanmean(global_p2fs)
                std_p2f = np.nanstd(global_p2fs)
            ...
            if global_uniform:
                ...
                uniform_mean = np.mean(global_uniforms, axis=0)
                for i in range(precentages.shape[0]):
                    row["uniform_%d" % i] = uniform_mean[i, 0]*1000.
            print(" | ".join(["{:>15.8f}".format(d) for d in row.values()]))

            with open(os.path.join(FLAGS.save_path, "finalresult.text"), "w") as text:
                print(row, file=text)
