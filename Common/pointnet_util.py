...
def sample_and_group(npoint, radius, nsample, xyz, points, tnet_spec=None, knn=False, use_xyz=True):
    ...
    if knn:
        ...
        if np.isscalar(radius):
            ...
            idx_list = []
            for radius_one, xyz_one, new_xyz_one in zip(tf.unstack(radius, axis=0), tf.unstack(xyz, axis=0),
                                                        tf.unstack(new_xyz, axis=0)):
                idx_one, _ = query_ball_point(radius_one, nsample, tf.expand_dims(xyz_one, axis=0),
                                              tf.expand_dims(new_xyz_one, axis=0))
...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training,
                       bn_decay, scope, bn=False, ibn=False, pooling='max', tnet_spec=None, knn=False, use_xyz=True):
    ...
    with tf.variable_scope(scope) as sc:
        if group_all:
            ...
            new_xyz, new_points, idx, grouped_xyz = sample_and_group(npoint, radius, nsample, xyz, points, tnet_spec,
                                                                     knn, use_xyz)
        if mlp2 is None: mlp2 = []
        for i, num_out_channel in enumerate(mlp):
            new_points = ops.conv2d(new_points, num_out_channel, [1, 1],
...
def pointnet_sa_module_msg(xyz, points, npoint, radius_list, nsample_list, mlp_list, is_training, bn_decay, scope,
                           bn=False, ibn=False, use_xyz=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i in range(len(radius_list)):
            ...
            for j, num_out_channel in enumerate(mlp_list[i]):
                grouped_points = ops.conv2d(grouped_points, num_out_channel, [1, 1],
                                            padding='VALID', stride=[1, 1], bn=bn, ibn=ibn,
...
def pointnet_fp_module(xyz1, xyz2, points1, points2, mlp, is_training, bn_decay, scope, bn=False, ibn=False):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            new_points1 = ops.conv2d(new_points1, num_out_channel, [1, 1],
                                     padding='VALID', stride=[1, 1],
                                     bn=bn, ibn=ibn, is_training=is_training,
